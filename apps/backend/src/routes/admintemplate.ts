...
import { audioUpload, extFromMime, videoUpload } from "../lib/upload.js";
...
async function resolveBlockFace(
    ...
    return avatar?.faceKey ? (await downloadObject(avatar.faceKey)) as Buffer ?? null : null;
}
...
adminTemplatesRouter.post(
    ...
    async (req: AuthedRequest, res) => {
        ...
        const swapSide = async (frameKey: string) => {
            ...
            if (!base) throw new Error("Failed to download frame");
            const swapped = await applyFaceSwap(face, base as Buffer, sniff(base as Buffer), {
...
adminTemplatesRouter.post(
    ...
    async (req: AuthedRequest, res) => {
        ...
        try {
            ...
            if (!clip) { res.status(500).json({ error: "Failed to download clip" }); return; }
            const frame = await generateThumbnail(clip as Buffer, parsed.data.atSec);
