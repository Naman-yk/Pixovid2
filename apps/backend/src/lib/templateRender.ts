...
export async function renderBlockClip(
    ...
    const prepareFrame = async (
        ...
        if (swap && opts.useSwapCache && swappedKey) {
            ...
            return { buffer: cached as Buffer, mime: sniffImageMime(cached as Buffer) };
        }
        const buffer = await downloadObject(key);
        if (!buffer) return null;
        const mime = sniffImageMime(buffer as Buffer);
        if (swap && face) {
            return applyFaceSwap(face, buffer as Buffer, mime, {
                swapModel: block.swapModel,
                context: block.swapContext,
                aspectRatio: block.aspectRatio,
            });
        }
        return { buffer: buffer as Buffer, mime };
    };
...
export async function buildBlockLipsyncAudio(
    ...
    const audioParts: AudioWindowPart[] = await Promise.all(
        overlapping.map(async (p) => ({
            buffer: (await downloadObject(p.audioKey)) as Buffer,
            inPoint: p.inPoint,
            length: p.length,
            delaySec: p.delaySec,
        })),
    );
    const mixed = await mixAudioWindow(audioParts, windowLen);
...
export async function renderTemplate(params: {
    ...
    await Promise.all(
        uniqueSlots.map(async (slot) => {
            ...
            if (avatar?.faceKey) {
                const buffer = await downloadObject(avatar.faceKey);
                faceCache.set(slot, buffer ? (buffer as Buffer) : null);
            } else {
        ...
        try {
            ...
            if (block.videoKey) {
                ...
                if (buffer) {
                    ...
                    return { buffer: buffer as Buffer, cost: 0 };
                }
            }
    ...
    const clips: Buffer[] = clipResults.map((r) => r.buffer as Buffer);
    ...
    const audioParts = await Promise.all(
        audioClips.map(async (clip) => {
            ...
            return {
                buffer: (await downloadObject(clip.audioKey)) as Buffer,
